...
void Analyzer::FitMass(){
	...
	TF1 *bw = new TF1("bw","([0]*[1])/((x*x - [2]*[2])*(x*x - [2]*[2]) + 0.25*[1]*[1])",110,150);
	...
	c2->cd(2);
	higgs->Add(qqZZ);
	higgs->GetXaxis()->SetRangeUser(110.0,150.0);

	higgs->Fit(total, "l");
	higgs->Draw("pe1x0");
   	c2->SaveAs("higgs_fit.pdf");
...
void Analyzer::Loop(TString fileName)
{
   ...
   h1->Fill(LepPt->at(0));
	...
	if(fileName=="ggH125")	{	higgs->Fill(h.M(),omega);
	higgs_signal->Fill(D,omega);
	masa_kin_sig->Fill(h.M(), D,omega);
}
	
	if(fileName=="qqZZ")	{
	qqZZ->Fill(h.M(),omega);
	qqZZ_poz->Fill(D,omega); }
